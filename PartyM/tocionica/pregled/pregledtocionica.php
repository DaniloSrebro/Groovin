...
<head>
  ...
  <!-- Include moment.js and the moment adapter for Chart.js -->
...
tr td:nth-child(1),tr th:nth-child(1){border-radius: .625rem .625rem .625rem .625rem;}

tr td:nth-child(2),tr th:nth-child(2){border-radius: .625rem 0 0 .625rem;}
...
#toggleChart {
  ...
  z-index: 200;
    ...
    color: #bbc0c6; /* Primary color for the icon */
    transition: color 0.3s ease, transform 0.3s ease; /* Smooth transition for color and scale */
    margin: 10px; /* Adds space around the icon */
    cursor: pointer; /* Changes the cursor to pointer on hover */
}

#toggleChart:hover {
    color: #6b2ca6; /* Changes color on hover */
    transform: scale(1.1); /* Slightly enlarges the icon on hover */
}

#toggleChart:active {
    transform: scale(0.95); /* Slightly shrinks the icon when clicked */
}
...
<div id="preloader">
  ...
  <br>
    ...
    <a href="#"><button class="button1" id="todayBtn" style="background-color: #ffc506;">Today</button><br><p style="color:white; margin-left: 48%;"><?php echo $row_count_variable; ?></p></a>
...
<i class="bi bi-graph-up" id="toggleChart" style="cursor: pointer;"></i>
...
<div class="chart-container" id="chartContainer" style="display: none;">
  ...
  <div id="noDataMessage">Nema rezervacija za danas.</div>
</div>
...
if ($result->num_rows > 0) {
  ...
  echo "<thead>
                <tr>
                  <th style='text-align:center;'>Za</th>
                  <th>Info</th>
                  <th>Sto</th>
                  <th>Osobe</th>
                  <th>Dolazak</th>
      ...
      while ($row = $result->fetch_assoc()) {
        ...
        // Get the reservation time from the database
        $reservationTime = new DateTime($row['vremedolaska']);
    
        // Calculate the difference
        $timeDifference = $currentTime->diff($reservationTime);
        ...
        if ($reservationTime > $currentTime) {
          if ($totalMinutes <= 10) {
            ...
            $textColor = "black"; // Use black for light backgrounds
        } elseif ($totalMinutes <= 20) {
            $bgColor = "#fff5cc"; // Light yellow
            $textColor = "black";
        } elseif ($totalMinutes <= 30) {
            $bgColor = "#ffffcc"; // Light cream
            $textColor = "black";
        } elseif ($totalMinutes <= 40) {
            $bgColor = "#ccffcc"; // Light green
            $textColor = "black";
        } elseif ($totalMinutes <= 50) {
            $bgColor = "#99ffcc"; // Pale green
            $textColor = "black";
        } elseif ($totalMinutes <= 60) {
            $bgColor = "#66ffcc"; // Light teal
            $textColor = "black";
        } elseif ($totalMinutes <= 70) {
            $bgColor = "#33cccc"; // Teal
            $textColor = "white"; // Use white for darker backgrounds
        } elseif ($totalMinutes <= 80) {
            $bgColor = "#00cccc"; // Dark teal
            $textColor = "white";
        } elseif ($totalMinutes <= 90) {
            $bgColor = "#0099cc"; // Light blue
            $textColor = "white";
        } elseif ($totalMinutes <= 100) {
            $bgColor = "#0066cc"; // Medium blue
            $textColor = "white";
        } elseif ($totalMinutes <= 110) {
            $bgColor = "#0033cc"; // Dark blue
            $textColor = "white";
        } elseif ($totalMinutes <= 120) {
            $bgColor = "#0000cc"; // Deep blue
            $textColor = "white";
        } elseif ($totalMinutes <= 130) {
            $bgColor = "#0000ff"; // Bright blue
            $textColor = "white";
        } elseif ($totalMinutes <= 140) {
            $bgColor = "#b3ccff"; // Light icy blue
            $textColor = "black"; // Switch back to black for lighter icy blue
        } elseif ($totalMinutes <= 150) {
            $bgColor = "#99ccff"; // Icy blue
            $textColor = "black";
        } elseif ($totalMinutes <= 160) {
            $bgColor = "#66b3ff"; // Light sky blue
            $textColor = "black";
        } elseif ($totalMinutes <= 170) {
            $bgColor = "#3399ff"; // Sky blue
            $textColor = "white"; // Use white for darker sky blue
        } elseif ($totalMinutes <= 180) {
            $bgColor = "#0099ff"; // Bright sky blue
            $textColor = "white";
        } else {
            $bgColor = "#d3d3d3"; // Light gray
            $textColor = "black"; // Use black for light gray
        }
      } else {
          $bgColor = "#333333";
          $icon = "⚠️";
          ...
          if ($reservationTime > $currentTime) {
            // Format as H hours and M minutes
            if ($totalMinutes >= 60) {
                $hours = floor($totalMinutes / 60);
                $minutes = $totalMinutes % 60;
                echo "<td style='text-align:center;background-color: $bgColor !important; color:$textColor !important;'>" 
    . $hours . "s " 
    . $minutes . "m" 
    . (!empty($row["kasnjenje"]) ? " +" . $row["kasnjenje"] : "") 
    . "</td>";
            } else {
              echo "<td style='text-align:center;background-color: $bgColor !important; color:$textColor !important;'>" 
              . $totalMinutes . "m" 
            ...
            <div>" . $time->format('H:i') . "</div>
                  ...
                  <div class='containersmesti' style='display: flex; align-items: center; gap: 10px;'>
                      ...
                      <div class='popuppromeni' style='display: none; font-size: 20px; color:black;'>
                          <form method='post' action='../akcije/podesi/podesi.php'>
                          ...
                          <br>
                          <p>ili</p>
                          <form method='post' action='../akcije/vrati/vrati.php'>
                              <input type='hidden' name='reservation_id' value='" . $row["id"] . "'>
                  ...
                  <div class='column-actions' style='display: flex; align-items: center; gap: 10px;'>
                      <form method='post' action='../akcije/bodovi/plusbod.php'>
                          <input type='hidden' name='stiglarez' value='" . $row["id"] . "'>
                          <button type='submit' name='stiglarez_button' class='icon-button'>
                              <a href='#' class='text-secondary mr-2'>
                                  <i class='material-icons-outlined approved-icon'>check_circle</i>
                              </a>
                          </button>
                      </form>
                      ...
                      <div class='containersmesti'>
                          <button onclick='toggleForm(this)' class='icon-button'>
                              <a href='#' class='text-danger ml-2'>
                                  <i class='material-icons-round rejected-icon'>delete_outline</i>
                              </a>
                          </button>
                          <div class='popuppromeni' style='display: none;font-size: 20px; color:black;'>
                              <form method='post' action='../akcije/bodovi/minusbod.php'>
                                  <label>Da li ste sigurni?</label><br><br>
                                  <input type='hidden' name='nijestiglarez' value='" . $row["id"] . "'>
                                  <button type='submit' name='nijestiglarez_button' class='nijestiglarez'>Da</button>
                              </form>
                          </div>
                      </div>
                  </div>
...
<script>
  ...
  async function fetchData() {
    try {
      const response = await fetch('./grafik/fetch_reservations.php');
      const data = await response.json();
      console.log('Fetched data:', data); // Log data to verify it's correct
      // Convert the data for Chart.js
      return data.map(item => ({
        x: new Date(item.vremedolaska), // Ensure this is a full date-time string
        y: item.brojljudi
      }));
    } catch (error) {
  ...
  async function createChart() {
    ...
    console.log('Formatted reservations:', reservations); // Log converted data to verify it's correct

    if (reservations.length === 0) {
      document.getElementById('noDataMessage').style.display = 'block';
      return; // Stop execution if there are no reservations
    } else {
    ...
    // Set the min and max times for the x-axis
    const minTime = new Date(year, month, date, 9, 0, 0);
    const maxTime = new Date(year, month, date, 23, 0, 0);
    ...
    const currentTimePoint = {
      x: currentTime,
      y: 0, // Arbitrary value since this point is only for the legend
      pointRadius: 0, // Hide the actual point
      pointHoverRadius: 0 // Hide the hover effect
    };
      ...
      options: {
        ...
        scales: {
          x: {
            ...
            min: minTime, // Set min time dynamically
            max: maxTime  // Set max time dynamically
          },
          y: {
            beginAtZero: true,
            title: {
            ...
            ticks: {
              ...
              stepSize: 1 // Increment step size to ensure integer values
            },
            grid: {
              color: '#bfbeb8'
            }
    ...
    reservationChart.render(); // Ensure the chart is rendered
  }
...
setInterval(reloadPage, 100000); // 60,000 milliseconds = 60 seconds
</script>
