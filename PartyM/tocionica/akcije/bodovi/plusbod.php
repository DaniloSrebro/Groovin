...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['stiglarez'])) {
    ...
    $copy_sql = "INSERT INTO rezervacije.tocionicaistorija (id, ime, vrstarez, brojtelefona, brojstola, brojljudi, reservationstatus, zadatum, vremerez, mesto, vremedolaska, mestosedenja, sedenje, dosli, email) 
                 SELECT id, ime, vrstarez, brojtelefona, brojstola, brojljudi, reservationstatus, zadatum, vremerez, mesto, vremedolaska, mestosedenja, sedenje, 'da' AS dosli, email FROM rezervacije.tocionica WHERE id = ? AND zadatum = CURDATE()";
