...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["reservation_id"])) {
    ...
    $copy_sql = "INSERT INTO rezervacije.tocionicaodbijene (id, ime, vrstarez, brojljudi, brojtelefona, brojstola, reservationstatus, zadatum, vremerez, mesto, vremedolaska, mestosedenja, sedenje, email, notification_sent) 
                 SELECT id, ime, vrstarez, brojljudi, brojtelefona, brojstola, 'rejected' AS reservationstatus, zadatum, vremerez, mesto, vremedolaska, mestosedenja, sedenje, email, '0' AS notification_sent FROM rezervacije.tocionica WHERE id = ? AND zadatum = DATE_ADD(CURDATE(), INTERVAL 1 DAY)";
    ...
    $delete_sql = "DELETE FROM rezervacije.tocionica WHERE id = ? AND zadatum = DATE_ADD(CURDATE(), INTERVAL 1 DAY)";
