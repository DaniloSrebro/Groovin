...
<div id="preloader">
  ...
  <br>
    ...
    <a href="#"><button class="button1" id="todayBtn" style="background-color: rgba(133, 131, 131, 0.5); border: 2px solid #ccc; color: white; border-radius: 20px; cursor: pointer;">Today</button>
    <a href="./1tocionicamenadzer.php"><button class="button1" id="tomorrowBtn">Tomorrow</button></a>
    <a href="./2tocionicamenadzer.php"><button class="button1" id="dayAfterTomorrowBtn">Day after Tomorrow</button></a>
      ...
      while ($row = $result->fetch_assoc()) {
        ...
        <button onclick='ukljuciForm(this)' class='reject-button'><i class='bi bi-ban'></i></button>
          <div class='form-container-odbi' style='display: none;'>
              <form method='post' action='../akcije/odbij/odbij.php'>
                ...
                <div class='containersmesti'>
                    ...
                    <div class='form-container' style='display: none;'>
                        <form method='post' action='../akcije/odobri/odobri.php'>
                            <input type='hidden' name='reservation_id' value='" . $row["id"] . "'>
...
<div class="col">
    ...
    <?php
        ...
        $sql = "SELECT brojstola, vremedolaska FROM tocionica WHERE reservationstatus='approved' AND zadatum = CURDATE() ORDER BY vremedolaska asc";
...
<br><br><br><br><br><br><br><br><br><br><br>
  ...
  <script>
    // Function to set the table number in the form that is currently displayed
function setTableNumber(button) {
    // Find the form container that is currently displayed
    const formContainer = document.querySelector('.form-container[style="display: block;"]');
    
    if (formContainer) {
        const tableNumber = button.getAttribute('data-table');
        const tableNumberInput = formContainer.querySelector('#inputField');
        tableNumberInput.value = tableNumber;
    } else {
        alert('No form is currently open.');
    }
}

// Add event listeners to table number buttons
...
<!-- URADJENO ONLAJN  -->
  <!-- <script>  //OVO SAM URADIO ZA SVIH 6 DANA ONLAJN, a na lokalu nisam mrzi me
  // Select all buttons with the class 'btn' using a different variable name, like 'btnElements'   //OVO SAM URADIO ZA SVIH 6 DANA ONLAJN, a na lokalu nisam mrzi me
...
document.querySelectorAll('.btn').forEach(button => {
    ...
    // Add this event listener for mouseout
     button.addEventListener('mouseout', function() {     //ZA VRACANJE MISA, GASENJE POPUP-a ZA VEC NAPRAVLJENE REZERVACIJE U COSKU
...
function fetchReservations(tableNumber) {
    ...
    xhr.open('GET', 'fetch_reservations.php?table_number=' + tableNumber);
    xhr.onload = function() {
        if (xhr.status === 200) {
...
<script>
  function toggleForm(button) {   //Za dodavanje stola
...
function checkForNewReservation1() {
    ...
    $sql = "SELECT * FROM tocionica WHERE reservationstatus = 'pending' AND zadatum = DATE_ADD(CURDATE(), INTERVAL 1 DAY)";
    $result = $conn->query($sql);

    // Output JavaScript code based on the query result
    echo "if (" . $result->num_rows . " > 0) {";
      echo "  document.getElementById('tomorrowBtn').classList.add('shine');"; // Change '.glowing' to '.shine'
      echo "} else {";
      echo "  document.getElementById('tomorrowBtn').classList.remove('shine');"; // Change '.glowing' to '.shine'
      echo "}";
...
<script>
function checkForNewReservation2() {
    <?php
    // Connect to your database
    $servername = "localhost";
    $username = "root";
    ...
    $sql = "SELECT * FROM tocionica WHERE reservationstatus = 'pending' AND zadatum = DATE_ADD(CURDATE(), INTERVAL 2 DAY)";
    $result = $conn->query($sql);

    // Output JavaScript code based on the query result
    echo "if (" . $result->num_rows . " > 0) {";
      echo "  document.getElementById('dayAfterTomorrowBtn').classList.add('shine');"; // Change '.glowing' to '.shine'
      echo "} else {";
      echo "  document.getElementById('dayAfterTomorrowBtn').classList.remove('shine');"; // Change '.glowing' to '.shine'
      echo "}";
...
// Check for updates every 5 seconds
setInterval(checkForNewReservation2, 1000);
</script>

<script>
function checkForNewReservation3() {
    <?php
    // Connect to your database
    $servername = "localhost";
    $username = "root";
    ...
    $sql = "SELECT * FROM tocionica WHERE reservationstatus = 'pending' AND zadatum = DATE_ADD(CURDATE(), INTERVAL 3 DAY)";
    $result = $conn->query($sql);

    // Output JavaScript code based on the query result
    echo "if (" . $result->num_rows . " > 0) {";
      echo "  document.getElementById('day3Btn').classList.add('shine');"; // Change '.glowing' to '.shine'
      echo "} else {";
      echo "  document.getElementById('day3Btn').classList.remove('shine');"; // Change '.glowing' to '.shine'
      echo "}";
...
// Check for updates every 5 seconds
setInterval(checkForNewReservation3, 1000);
</script>

<script>
function checkForNewReservation4() {
    <?php
    // Connect to your database
    $servername = "localhost";
    $username = "root";
    ...
    $sql = "SELECT * FROM tocionica WHERE reservationstatus = 'pending' AND zadatum = DATE_ADD(CURDATE(), INTERVAL 4 DAY)";
    $result = $conn->query($sql);

    // Output JavaScript code based on the query result
    echo "if (" . $result->num_rows . " > 0) {";
      echo "  document.getElementById('day4Btn').classList.add('shine');"; // Change '.glowing' to '.shine'
      echo "} else {";
      echo "  document.getElementById('day4Btn').classList.remove('shine');"; // Change '.glowing' to '.shine'
      echo "}";
...
// Check for updates every 5 seconds
setInterval(checkForNewReservation4, 1000);
</script>

<script>
function checkForNewReservation5() {
    <?php
    // Connect to your database
    $servername = "localhost";
    $username = "root";
    ...
    $sql = "SELECT * FROM tocionica WHERE reservationstatus = 'pending' AND zadatum = DATE_ADD(CURDATE(), INTERVAL 5 DAY)";
    $result = $conn->query($sql);

    // Output JavaScript code based on the query result
    echo "if (" . $result->num_rows . " > 0) {";
      echo "  document.getElementById('day5Btn').classList.add('shine');"; // Change '.glowing' to '.shine'
      echo "} else {";
      echo "  document.getElementById('day5Btn').classList.remove('shine');"; // Change '.glowing' to '.shine'
      echo "}";
...
// Check for updates every 5 seconds
setInterval(checkForNewReservation5, 1000);
</script>

<script>
function checkForNewReservation6() {
    <?php
    // Connect to your database
    $servername = "localhost";
    $username = "root";
    ...
    $sql = "SELECT * FROM tocionica WHERE reservationstatus = 'pending' AND zadatum = DATE_ADD(CURDATE(), INTERVAL 6 DAY)";
    $result = $conn->query($sql);

    // Output JavaScript code based on the query result
    echo "if (" . $result->num_rows . " > 0) {";
      echo "  document.getElementById('day6Btn').classList.add('shine');"; // Change '.glowing' to '.shine'
      echo "} else {";
      echo "  document.getElementById('day6Btn').classList.remove('shine');"; // Change '.glowing' to '.shine'
      echo "}";
...
// Check for updates every 5 seconds
setInterval(checkForNewReservation6, 1000);
</script>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('zadatum').value = today;
    });
</script>
