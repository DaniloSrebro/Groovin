...
if (isset($_SESSION["user_id"]) && isset($_GET['id'])) {
    // Retrieve user ID and DJ ID from session and query string
    $user_id = $_SESSION["user_id"];
    $dj_id = $_GET['id']; // This is the DJ ID passed via the URL

    // Query to check if the user has liked the DJ with the specified ID
    $sql_check_like = "SELECT * FROM mikula WHERE user_id = ? AND dj_id = ?";
    $stmt_check_like = $conn_djlikes->prepare($sql_check_like);
    $stmt_check_like->bind_param("ii", $user_id, $dj_id);
...
// Check if `dj_id` is provided in the URL
if (isset($_GET['id']) && !empty($_GET['id'])) {
    $id = intval($_GET['id']); // Sanitize input
    
    // Fetch DJ details from the database
    $sql = "SELECT * FROM dj WHERE id = ?";
...
.song-modal-content {
  ...
  left: 50%;
  transform: translate(-50%, -50%); /* Perfectly center by shifting back */
...
.close {
  ...
  right: 15px;
  font-size: 24px;
  color: #4b137d; /* Your main site color */
  cursor: pointer;
  transition: color 0.2s ease;
}

.close:hover {
  color: #d11a5f; /* Slightly brighter on hover */
}
...
#song-search:focus {
  border-color: #4b137d;
  box-shadow: 0 0 5px rgba(75, 19, 125, 0.4);
}
...
<div class="container1 emp-profile">
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="profile-img">
                            <img src="uploads/profile_pictures/<?php echo htmlspecialchars($dj['profile_picture']); ?>" alt=""/>
        ...
        <script>
    var dj_id = <?php echo json_encode($dj_id); ?>;
        ...
        $('#Give-It-An-Id').on('change', function() {
            if (this.checked) {
                ...
                $.ajax({
                    url: 'like.php',
                    type: 'POST',
                    data: { action: 'like', id: dj_id }, // Sending the dj_id correctly
                    success: function(response) {
                        try {
                            var result = JSON.parse(response); 
                            if (result.status === 'liked') {
                                // Successfully liked the DJ
                            }
                        } catch (e) {
                    ...
                    error: function(xhr, status, error) {
                        ...
                        recenica.offsetHeight; // Trigger reflow
                ...
                $.ajax({
                    url: 'like.php',
                    type: 'POST',
                    data: { action: 'unlike', id: dj_id }, // Sending the dj_id correctly
                    success: function(response) {
                        try {
                            var result = JSON.parse(response); 
                            if (result.status === 'unliked') {
                                // Successfully unliked the DJ
                            }
                        } catch (e) {
                            console.error("Error parsing JSON:", e);
                        }
...
<h6>
                                    <?php echo htmlspecialchars($dj['profession'] ?? ''); ?>
                                    </h6>
...
$sql = "SELECT live FROM dj WHERE id = $dj_id";
...
<div class="live-status">
    <?php if ($live == 1): ?>
        <button class="button-6" id="openModalButton">Request a song</button>
...
if (isset($_SESSION["user_id"])) {
    ...
    $sql = "SELECT * FROM song_requests WHERE user_id = '$user_id' AND dj_id = '$dj_id' AND played = '0'";
    $result = $conn->query($sql);
...
<script>
// Get the dj_id from the PHP variable
var dj_id = <?php echo isset($_GET['id']) ? $_GET['id'] : 0; ?>; // Default to 0 if not set

// Function to make the AJAX request for song search
function searchSong() {
    var query = document.getElementById('song-search').value;

    // Check if query length is sufficient (at least 3 characters)
    if (query.length >= 3) {
        $.ajax({
            url: 'search_song.php',  // The PHP script to process the search request
            type: 'GET',
            data: { song: query, dj_id: dj_id },  // Pass dj_id from the PHP code to the AJAX request
            success: function(response) {
                // Display the response in the #search-results div
                        ...
                        <div class="tab-content profile-tab" id="myTabContent">
                            <div class="tab-pane fade" id="home" role="tabpanel" aria-labelledby="home-tab">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-6">
                                                <p><?php echo htmlspecialchars($dj['username'] ?? ''); ?></p>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-6">
                                            ...
                                            <div class="col-md-6">
                                                <p><?php echo htmlspecialchars($dj['real_name'] ?? ''); ?></p>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-6">
                                            ...
                                            <div class="col-md-6">
                                                <p><?php echo htmlspecialchars($dj['email'] ?? ''); ?></p>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-6">
                                            ...
                                            <div class="col-md-6">
                                                <p><?php echo htmlspecialchars($dj['phone_number'] ?? ''); ?></p>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-6">
                                            ...
                                            <div class="col-md-6">
                                                <p><?php echo htmlspecialchars($dj['profession'] ?? ''); ?></p>
                                            </div>
                                        </div>
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <label>Bio</label><br/>
                                                <p><?php echo htmlspecialchars($dj['bio'] ?? ''); ?></p>
                                            </div>
                                        </div>
                            </div>
                                        ...
                                        <div class="col-md-12">
                                            ...
                                            <table class="table">
                                                ...
                                                <tbody>
                                                    <tr>
                                                        <td>Monday</td>
                                                        <td><?php echo htmlspecialchars($dj['ponedeljak_klub'] ?? ''); ?></td>
                                                        <td><?php echo htmlspecialchars($dj['ponedeljak_start'] ?? ''); ?></td>
                                                    </tr>
                                                    <tr>
                                                        <td>Tuesday</td>
                                                        <td><?php echo htmlspecialchars($dj['utorak_klub'] ?? ''); ?></td>
                                                        <td><?php echo htmlspecialchars($dj['utorak_start'] ?? ''); ?></td>
                                                    </tr>
                                                    <tr>
                                                        <td>Wednesday</td>
                                                        <td><?php echo htmlspecialchars($dj['sreda_klub'] ?? ''); ?></td>
                                                        <td><?php echo htmlspecialchars($dj['sreda_start'] ?? ''); ?></td>
                                                    </tr>
                                                    <tr>
                                                        <td>Thursday</td>
                                                        <td><?php echo htmlspecialchars($dj['cetvrtak_klub'] ?? ''); ?></td>
                                                        <td><?php echo htmlspecialchars($dj['cetvrtak_start'] ?? ''); ?></td>
                                                    </tr>
                                                    <tr>
                                                        <td>Friday</td>
                                                        <td><?php echo htmlspecialchars($dj['petak_klub'] ?? ''); ?></td>
                                                        <td><?php echo htmlspecialchars($dj['petak_start'] ?? ''); ?></td>
                                                    </tr>
                                                    <tr>
                                                        <td>Saturday</td>
                                                        <td><?php echo htmlspecialchars($dj['subota_klub'] ?? ''); ?></td>
                                                        <td><?php echo htmlspecialchars($dj['subota_start'] ?? ''); ?></td>
                                                    </tr>
                                                    <tr>
                                                        <td>Sunday</td>
                                                        <td><?php echo htmlspecialchars($dj['nedelja_klub'] ?? ''); ?></td>
                                                        <td><?php echo htmlspecialchars($dj['nedelja_start'] ?? ''); ?></td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                        </div>
